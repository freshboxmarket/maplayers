...
// Revision notes:
// - Pure JS (no <script> wrapper)
// - DOM-ready boot to avoid early #map/Leaflet race
// - Auto-create UI shell (#legend, #drivers, #status, #error) if missing
// - Always surface errors (creates a visible banner if #error doesn't exist)
// - Wires the "Highlight outside customers" toggle
// - Optional diagnostics banner (?diag=1)
// - Gentle #map height fallback if it renders with ~0 height

(function () {
  function start() {
    (async function () {
      // ---------- early, guaranteed error surface ----------
      ensureUiShell(); // creates #legend/#drivers/#status/#error if missing
      const cssFallback = document.createElement('style');
      cssFallback.textContent = `
        .top-error{position:fixed;left:12px;right:12px;top:12px;z-index:10000;background:#ffebee;color:#b71c1c;
          border:1px solid #ffcdd2;border-radius:8px;box-shadow:0 4px 12px rgba(0,0,0,.08);padding:10px;display:none;font:600 13px system-ui}
        #diag{position:fixed;right:12px;top:12px;z-index:9000;background:rgba(255,255,255,.95);border-radius:8px;
          box-shadow:0 6px 18px rgba(0,0,0,.12);padding:10px 12px;font:600 12px system-ui;line-height:1.35;display:none;max-width:40ch}
      `;
      ...
      // Ensure #map container exists; if not, create a basic one
      ensureMapRoot();
      // Minimal height fallback if map has 0 height
      const mapEl = document.getElementById('map');
      try {
        const h = parseInt(getComputedStyle(mapEl).height, 10);
        if (!isFinite(h) || h < 40) {
          const fix = document.createElement('style');
          fix.textContent = `#map{height:100vh}`;
      ...
      // helper to keep legend consistent everywhere
      function updateLegend() {
        ...
        renderLegend(cfg, legendCounts, custWithinSel, custOutsideSel, outsideHighlight);
      }
      ...
      function showTopError(title, msg) {
        ...
        if (!el) {
          el = document.querySelector('.top-error');
          if (!el) {
            el = document.createElement('div');
            el.className = 'top-error';
      ...
      async function applySelection() {
        ...
        if (noKeys) {
          ...
          // If nothing matched visibly, warn once
          if (selectedSet.size > 0 && coveragePolysSelected.length === 0) {
            warn('Selected keys did not match any loaded polygons. Check cfg.fields.key and your GeoJSON properties.');
          }
        }
      ...
      function recolorAndRecountCustomers() {
        ...
        // default outside style (dim)
        let outStyle = { radius: cst.radius || 9, color: '#7a7a7a', weight: cst.weightPx || 2, opacity: 0.8, fillColor: '#c7c7c7', fillOpacity: 0.6 };
        // highlight outside style (emphasized) if enabled
        if (outsideHighlight) {
          outStyle = { radius: cst.radius || 9, color: '#d32f2f', weight: cst.weightPx || 2, opacity: 0.95, fillColor: '#ffcdd2', fillOpacity: 0.95 };
        }
        ...
        renderDriversPanel(driverMeta, driverOverlays, true, driverSelectedCounts, custWithinSel);
        updateLegend(); // keep toggle state synced
      }
      ...
      function renderLegend(cfg, legendCounts, custIn, custOut, outsideToggle) {
        ...
        if (tgl) {
          tgl.addEventListener('change', (e)=>{
            ...
            recolorAndRecountCustomers(); // will call updateLegend internally
          });
        }
      }

      // ---------- Diagnostics (opt-in via ?diag=1) ----------
      function updateDiagnostics() {
        ...
        const haveMap = !!document.getElementById('map');
        const haveLeaflet = (typeof L !== 'undefined');
        const featureCounts = (arr) => (arr||[]).reduce((a,e)=>a+(e.features?.length||0),0);
        const baseCnt = featureCounts(baseDayLayers);
        const quadCnt = featureCounts(quadDayLayers);
        const subqCnt = featureCounts(subqDayLayers);
        const selKeys = selectedOrderedKeys.length;
        const selPolys = coveragePolysSelected.length;

        box.style.display = 'block';
        box.innerHTML = [
          `cfg: ${escapeHtml(cfgUrl)}`,
          `#map: ${haveMap ? 'yes' : 'no'}`,
          `Leaflet: ${haveLeaflet ? 'yes' : 'no'}`,
          `features: base=${baseCnt} quad=${quadCnt} subq=${subqCnt}`,
          `batch routes: ${batchItems.length}`,
          `selected keys: ${selKeys} → matched polys: ${selPolys}`,
          `customers: total=${customerCount} in=${custWithinSel} out=${custOutsideSel}`
        ].map(x=>`<div>${x}</div>`).join('');
      }

      // --------------------------------------------------------------
      // Shell helpers: ensure UI containers so errors/status always show
      // --------------------------------------------------------------
      function ensureUiShell() {
        const need = [
          ...
          { id: 'drivers' },
          { id: 'status' },
          { id: 'error', className: 'top-error' }
        ];
        for (const spec of need) {
          if (!document.getElementById(spec.id)) {
            const n = document.createElement('div');
            n.id = spec.id;
            if (spec.className) n.className = spec.className;
